...
function queryRemoteBranches() {
  return command('git branch -r')
    .then(res => res.map(branchName => branchName.replace(/\s+origin\//, '')))
    .catch(() => [])
}
...
function queryLocalBranches () {
  return command('git branch').then(names => names.map(name => name.replace(/[\s|*]/g, '')))
}
...
function isRemoteBranch(branchName) {
  return queryRemoteBranches().then(names => names?.findIndex?.(n => n === branchName) !== -1)
}
...
function command (cmd) {
  return exec(cmd)
    ...
    .catch(({stdout}) => {throw new Error(stdout) })
}
